...
#         return json.load(f)
"""
Ensemble model: Random Forest + XGBoost for 10-day market direction prediction
with explanation layer + decision engine.
"""
...
def _generate_explanation(signal, confidence, features):
    parts = []

    if signal == "BULLISH":
        parts.append(f"Bullish trend detected ({confidence}% confidence).")
    else:
        parts.append(f"Bearish pressure detected ({confidence}% confidence).")

    rsi = features.get("rsi_14")
    if rsi:
        if rsi > 70:
            parts.append("RSI overbought → possible correction.")
        elif rsi < 30:
            parts.append("RSI oversold → rebound possible.")

    macd = features.get("macd")
    if macd:
        parts.append("MACD positive → trend strength." if macd > 0 else "MACD negative → weak trend.")

    vix = features.get("vix_close")
    if vix:
        parts.append("High VIX → risky." if vix > 20 else "Low VIX → stable.")

    return " ".join(parts)

def _decision_engine(signal, confidence, features):
    if confidence >= 70:
    ...
    risk = "Medium"
    if features.get("vix_close", 0) > 22:
        risk = "High"

    if signal == "BULLISH" and confidence > 65:
        action = "Buy"
...
def train(symbol: str, period: str = "2y") -> ModelResult:
    ...
    tscv = TimeSeriesSplit(n_splits=5)
    cv_scores = cross_val_score(model, scaler.transform(X), y, cv=tscv, scoring="accuracy", n_jobs=1)
...
def predict(symbol: str, retrain_if_missing=True) -> PredictResult:
    ...
    snapshot = df[feats].iloc[-1].to_dict()

    explanation = _generate_explanation(signal, confidence, snapshot)
    action, band, risk = _decision_engine(signal, confidence, snapshot)
    ...
    return PredictResult(
        ...
        xgb_probability=round(prob * 100, 1),
        horizon="10-day",
        sentiment_score=round(_sentiment_score(yf.Ticker(symbol)), 3),
        feature_snapshot=snapshot,
        shap_explanation=[],
        explanation_model="ensemble",
